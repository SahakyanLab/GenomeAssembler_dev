...
Rcpp::List calc_breakscore(const std::vector<std::string> &path, 
    ...
    std::vector<int> path_prob_dist_startpos(path.size());
    ...
    for(int i = 0; i < path.size(); i++){
        ...
        for(const auto &kv : read_matrix){
            ...
            if(pos != std::string::npos){
                ...
                int substring_startpos = true_solution.find(path[i]);
                path_prob_dist_startpos[i] = substring_startpos;
            }
        }
    ...
    std::vector<int> each_path_prob_dist_startpos(path.size());
    ...
    for(int i = 0; i < idx.size(); i++){
        ...
        each_path_prob_dist_startpos[i] = path_prob_dist_startpos[i];
    ...
    return Rcpp::List::create(
        ...
        Rcpp::Named("path_prob_dist_startpos") = each_path_prob_dist_startpos,
        Rcpp::Named("path_prob_dist") = Rcpp::wrap(sorted_path_prob_dist)
    );

    // // if aligner outputted multiple contigs instead of one scaffold,
    // // then sum all the metrics
    // double summed_break_score_vector = 0;
    // double summed_norm_break_score_vector = 0;
    // double summed_break_score_norm_by_len_vector = 0;
    // double summed_nr_of_breaks_vector = 0;
    // int summed_path_len = 0;
    // int lev_dist_vs_true = 0;
    // std::vector<std::vector<double>> each_path_prob_dist(path.size());
    // std::vector<int> each_path_prob_dist_startpos(path.size());
    // for(int i = 0; i < path.size(); i++){
    //     summed_break_score_vector += break_score_vector[i];
    //     summed_norm_break_score_vector += norm_break_score_vector[i];
    //     summed_break_score_norm_by_len_vector += break_score_norm_by_len_vector[i];
    //     summed_nr_of_breaks_vector += nr_of_breaks_vector[i];
    //     summed_path_len += path_len[i];
    //     each_path_prob_dist[i] = path_prob_dist[path[i]];
    //     each_path_prob_dist_startpos[i] = path_prob_dist_startpos[i];
        
    //     // calculate levenshtein distance of assembled vs true solution
    //     int lev_dist = calc_levenshtein(path[i], true_solution);
    //     lev_dist_vs_true += lev_dist;
    // }

    // return Rcpp::List::create(
    //     Rcpp::Named("sequence_len") = summed_path_len,
    //     Rcpp::Named("bp_score") = summed_break_score_vector,
    //     Rcpp::Named("bp_score_norm_by_break_freqs") = summed_norm_break_score_vector,
    //     Rcpp::Named("bp_score_norm_by_len") = summed_break_score_norm_by_len_vector,
    //     Rcpp::Named("kmer_breaks") = summed_nr_of_breaks_vector,
    //     Rcpp::Named("lev_dist_vs_true") = lev_dist_vs_true,
    //     Rcpp::Named("path_prob_dist_startpos") = each_path_prob_dist_startpos,
    //     Rcpp::Named("path_prob_dist") = Rcpp::wrap(each_path_prob_dist)
    // );
}