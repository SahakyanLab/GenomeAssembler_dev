...
string.reconstruction <- function(len = len, G.cont = G.cont, C.cont = C.cont, A.cont = A.cont, 
                                  k = k, NCPU = NCPU, prob.type = "non-uniform"){
  source("BreakageProbScoring.R"); source("ReadsGenerator.R")
  # generate random genome sequence
  randSeq <- ReadsGenerator(len = len, G.cont = G.cont, C.cont = C.cont, A.cont = A.cont,
                            k.mer = 2, NCPU = NCPU, prob.type = "non-uniform")
  # traverse all paths in the rooted tree and construct the Eulerian paths
  output  <- eulerian.path(get.balance.count(debrujin.graph.from.kmers(kmer.composition(k = k))))
  ...
  # calculate the alignment scores per reconstructed genome against the reference sequence
  align   <- alignment.scoring(path = output, randSeq[[1]], NCPU = NCPU)
  return(list(paste(randSeq[[1]], collapse = ""), align, breakage.output))
}

setwd("/Volumes/Paddy_Backup/ProjectBoard_Patrick/02-Proof_of_principle/scripts")
out <- string.reconstruction(len = 1000, G.cont = 0.3, C.cont = 0.3, A.cont = 0.2, 
                             k = 10, NCPU = 2, prob.type = "non-uniform")

##########################################################################################
library("ggplot2")
alignment.scores <- out[[2]][[2]]

df <- data.frame("alignment.scores" = alignment.scores,
                 "prob.ref.sums"    = out[[3]][[2]])

r.squared <- function(x, y){summary(lm(x~y))$r.squared}
rsq <- r.squared(df$alignment.scores, df$prob.ref.sums)
eq  <- bquote(R^2 == .(round(rsq, 5)))
p <- ggplot(df, aes(x = alignment.scores, y = prob.ref.sums)) + geom_point()
p + labs(x = "Alignment Scores", y = "Breakage Probability Scores") +
  annotate("text", x = -1500, y = 9000, label = eq)
##########################################################################################
